...
| `/blog` | `blog/BlogListPage.js` | Blog post listing — hero rotates random featured image as background, RSS feed link | `blog_posts` (published) |
| `/blog/:slug` | `blog/BlogPage.js` | Single blog post — featured image as hero banner with dark overlay, doc links at bottom | `blog_posts` (published) |
...
| Route | File | Purpose | DB Tables | Edge Functions |
|-------|------|---------|-----------|----------------|
| `/inbox` | `inbox/index.js` | Main messaging UI — SMS, calls, chat, email. Email filter pill, email threads grouped by thread_id, email thread view (prefers `body_html` with sanitization, falls back to `body_text`), compose with WYSIWYG toolbar (B/I/U/Link/List/Attach) + inline image upload, video attachment support. Split into `call-interface.js`, `listeners.js`, `messaging.js`, `views.js`, `voice-loader.js` | `sms_messages`, `call_records`, `chat_sessions`, `contacts`, `service_numbers`, `agent_configs`, `email_messages` | `send-email` (real-time subs) |
...
|------|---------|-------------------|
| `BottomNav.js` | Mobile bottom nav / desktop sidebar with unread badges, "Give Feedback" button, contact modal (category dropdown → `send-contact-email` → support ticket creation, BroadcastChannel live update to support tab), profile menu with Status modal, upgrade request | `sms_messages`, `call_records` |
...
| `formatters.js` | Shared utilities: `escapeHtml`, `formatPhoneNumber`, `getInitials` — consolidated from 20+ inline copies |
...
| `get-cloned-voices` | JWT | `cloned_voices` | ElevenLabs TTS | voice selection (generates TTS previews for cloned voices, stores in Supabase storage) |
...
| `fetch-agent-avatar` | JWT | `agent_configs` | ElevenLabs | agent config (looks up stored voice avatars in Supabase storage, falls back to ElevenLabs API for cloned voices) |
...
| Function | Auth | Tables | External APIs | Called By |
|----------|------|--------|---------------|----------|
| `knowledge-source-add` | JWT | `knowledge_sources`, `agent_configs` | Firecrawl, Jina Reader, Microlink, OpenAI | knowledge tab. Persists `auth_headers` to DB for re-syncs |
...
| `knowledge-source-sync` | JWT | `knowledge_sources`, `knowledge_chunks` | Firecrawl, Jina Reader, Microlink, OpenAI | knowledge tab. Uses stored `auth_headers` + JS fallback cascade |
...
| `publish-blog-to-twitter` | Service role | `blog_posts`, `twitter_oauth_tokens` | Twitter/X API v2 (tweets + media upload) | admin page, CLI |
| `twitter-oauth-callback` | Public / JWT | `twitter_oauth_tokens`, `twitter_oauth_state` | Twitter/X OAuth 2.0 | admin page |
| `generate-blog-image` | JWT | `blog_posts` | OpenAI DALL-E | admin page |
...
| Function | Auth | Tables | External APIs | Called By |
|----------|------|--------|---------------|----------|
| `send-email` | JWT | `email_messages`, `user_integrations` | Gmail API (OAuth) | inbox page. Accepts `attachments` array, embeds in HTML body, stores in `email_messages.attachments` JSONB |
...
| `gmail-push-webhook` | Pub/Sub secret | `agent_email_configs`, `email_messages`, `user_integrations`, `contacts`, `support_tickets`, `support_email_config`, `support-attachments` (storage) | Gmail API (messages + attachments), OpenAI | Google Cloud Pub/Sub. Two paths: (1) active `agent_email_configs` → processes into `email_messages`; (2) support inbox match → triggers `poll-gmail-tickets` |
| `gmail-watch-renew` | Service role | `agent_email_configs`, `user_integrations` | Gmail API (watch) | cron job (daily) |

### Support & Ticketing

**Email ticket flow**: help@magpipe.ai → alias → help@webrtc.is (Gmail) → Pub/Sub push → `gmail-push-webhook` (detects support inbox via `support_email_config`) → `poll-gmail-tickets` → dual-writes to `support_tickets` + `email_messages` (Inbox sync) with TKT-XXXXXX ref → AI auto-reply via Gmail API (when `agent_mode=auto`, also cross-written to `email_messages`). Single response per inbound email (no separate acknowledgment in auto mode).

| Function | Auth | Tables | External APIs | Called By |
|----------|------|--------|---------------|----------|
| `support-tickets-api` | JWT | `support_tickets`, `support_email_config`, `support-attachments` (storage) | Gmail API, Postmark, GitHub API | admin page. `send_reply` accepts `replyBodyHtml` + `attachments`; sends as HTML via Gmail/Postmark |
| `poll-gmail-tickets` | Service role | `support_tickets`, `email_messages`, `support_email_config`, `agent_email_configs`, `user_integrations`, `contacts`, `support-attachments` (storage) | Gmail API (messages + attachments), OpenAI (AI draft/auto-reply), Gmail API (send acknowledgment/reply) | `gmail-push-webhook` (primary), manual trigger. Normalizes outbound `from_email` to `send_as_email` |
...
### Knowledge & AI
`knowledge_sources` (auth_headers JSONB for authenticated crawling), `knowledge_chunks` (pgvector), `dynamic_variables`, `semantic_match_actions`, `custom_functions`
...
### Support
`support_tickets` (gmail_message_id, thread_id, ticket_ref TKT-XXXXXX, from/to email, subject, body, direction, status, labels[], ai_draft, ai_draft_status, attachments JSONB, priority, assigned_to, tags[], github_issue_url), `support_ticket_notes`, `support_email_config` (gmail_address, send_as_email, agent_mode auto/draft, support_agent_id, last_history_id for Gmail incremental sync)
...
| **ElevenLabs** | TTS, voice cloning | `preview-voice`, `clone-voice`, `list-voices`, `text-to-speech`, `get-cloned-voices`, `fetch-agent-avatar`, agent.py (eleven_flash_v2_5 for premade, eleven_multilingual_v2 for cloned voices) |
| **Twitter/X** | Social posting with images | `publish-blog-to-twitter` (v2 tweets + media upload), `twitter-oauth-callback` (OAuth 2.0 PKCE) |
...
| **Firecrawl** | Web scraping for KB (JS rendering, auth header forwarding) | `knowledge-source-add`, `knowledge-source-sync`, `knowledge-crawl-process` via `_shared/js-content-fetcher.ts` |
| **Jina Reader** | Free JS page rendering fallback | `knowledge-source-add`, `knowledge-source-sync`, `knowledge-crawl-process` via `_shared/js-content-fetcher.ts` |
| **Microlink** | Free JS page rendering fallback (last resort) | `knowledge-source-add`, `knowledge-source-sync`, `knowledge-crawl-process` via `_shared/js-content-fetcher.ts` |
