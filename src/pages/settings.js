        ...
        <div style="overflow-x: auto;">
          <table style="width: 100%; border-collapse: collapse; font-size: 0.85rem;">
            ...
            <tbody>
              ${keys.map(key => `
                <tr style="border-bottom: 1px solid var(--border-color);">
                  <td style="padding: 0.5rem 0.5rem 0.5rem 0; font-weight: 500; white-space: nowrap;">${escapeHtml(key.name)}</td>
                  ...
                  <td style="padding: 0.5rem; color: var(--text-secondary); white-space: nowrap;" class="api-keys-extra">
                    ${key.is_active ? (key.webhook_url
                      ? `<span style="font-size: 0.8rem; max-width: 180px; display: inline-block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; vertical-align: middle;" title="${escapeHtml(key.webhook_url)}">${escapeHtml(key.webhook_url)}</span>
                         <button class="btn btn-secondary api-key-edit-webhook-btn" data-key-id="${key.id}" data-webhook-url="${escapeHtml(key.webhook_url)}" data-webhook-secret="${escapeHtml(key.webhook_secret || '')}" style="font-size: 0.7rem; padding: 0.15rem 0.35rem; margin-left: 0.25rem; vertical-align: middle;">Edit</button>`
                  ...
                  <td style="padding: 0.5rem; text-align: right; white-space: nowrap;">
                    ${key.is_active
                      ? `<button class="btn btn-secondary api-key-revoke-btn" data-key-id="${key.id}" data-key-name="${escapeHtml(key.name)}" style="font-size: 0.75rem; padding: 0.25rem 0.5rem;">Revoke</button>`
      ...
      <div class="contact-modal" onclick="event.stopPropagation()" style="max-width: 500px;">
        ...
        <form id="webhook-edit-form">
          <div class="contact-modal-body">
            ...
            <input type="url" id="webhook-edit-url" value="${escapeHtml(currentUrl || '')}" placeholder="https://your-server.com/webhook" style="width: 100%;" maxlength="2048" />
