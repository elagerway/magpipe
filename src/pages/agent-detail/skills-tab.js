...
export const skillsTabMethods = {
  renderSkillsTab() {
    return `
      <!-- Saved forwarding numbers (per-user pick list) -->
      <div class="config-section">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.25rem;">
          <h3 style="margin: 0;">Saved forwarding numbers</h3>
          <button class="btn btn-sm btn-primary" id="fn-add-btn">+ Add</button>
        </div>
        <p class="section-desc">Reusable list of phone numbers you forward calls to. The one marked Default is pre-filled when you add a whitelist entry below, or when you mark an unknown caller as friendly from the inbox.</p>
        <div id="fn-list">
          ...
          <form id="whitelist-add-form">
            <div class="contact-modal-body">
              ...
              <div class="form-group">
                ...
                <input type="tel" id="whitelist-caller" class="form-input" placeholder="(604) 555-1234" required>
                <span class="form-hint" id="whitelist-caller-hint">Any format works — we'll normalize on save.</span>
              </div>
              <div class="form-group">
                <label class="form-label">Forward To <span style="color: var(--danger);">*</span></label>
                <select id="whitelist-forward-select" class="form-select">
                  <!-- populated by attachWhitelistTabListeners → loadForwardingNumbers -->
                  <option value="">— pick a saved number —</option>
                </select>
                <input type="tel" id="whitelist-forward-new" class="form-input" placeholder="(604) 555-9876" style="display: none; margin-top: 0.5rem;">
                <span class="form-hint" id="whitelist-forward-hint">Pick from saved numbers, or choose "Enter a different number" to add a new one (it'll be saved for next time).</span>
              </div>
            </div>
      ...
      <!-- Forwarding Number add/edit modal.
           Dismiss handlers are wired in JS (attachWhitelistTabListeners
           → fn-form binding) so we can gate backdrop / × / Cancel
           clicks on the saving flag and avoid closing mid-POST. -->
      <div class="contact-modal-overlay" id="fn-modal-overlay" style="display: none;">
        <div class="contact-modal" id="fn-modal-inner" style="max-width: 480px;">
          <div class="contact-modal-header">
            <h3 id="fn-modal-title">Add forwarding number</h3>
            <button type="button" class="close-modal-btn" id="fn-modal-close">&times;</button>
          </div>
          <form id="fn-form">
            <div class="contact-modal-body">
              <div class="form-group">
                <label class="form-label">Number <span style="color: var(--danger);">*</span></label>
                <input type="tel" id="fn-number" class="form-input" placeholder="(604) 555-1234" required>
                <span class="form-hint">Any format works — we'll normalize on save.</span>
              </div>
              <div class="form-group">
                <label class="form-label">Label <span style="color: var(--text-secondary); font-weight: normal;">(optional)</span></label>
                <input type="text" id="fn-label" class="form-input" placeholder="e.g. Mobile, Office" maxlength="100">
              </div>
              <div class="form-group">
                <label style="display: flex; align-items: center; gap: 0.5rem; cursor: pointer;">
                  <input type="checkbox" id="fn-is-default">
                  <span>Make this my default</span>
                </label>
                <span class="form-hint">Used by default when you whitelist a new caller or mark an unknown number as friendly from the inbox.</span>
              </div>
            </div>
            <div class="contact-modal-footer">
              <button type="button" class="btn btn-secondary" id="fn-modal-cancel">Cancel</button>
              <button type="submit" class="btn btn-primary" id="fn-save-btn">Save</button>
            </div>
          </form>
        </div>
      </div>
  ...
  async attachSkillsListeners() {
    // Load data. Forwarding numbers load first so the whitelist add
    // modal's forward-to combobox is populated by the time
    // loadWhitelist renders its hooks.
  ...
  // ── Forwarding Numbers (per-user pick list) ─────────────────────────────

  async loadForwardingNumbers() {
    ...
    try {
      ...
      this._forwardingNumbers = numbers;

      if (numbers.length === 0) {
        container.innerHTML = `<div style="color: var(--text-secondary); font-size: 0.875rem; padding: 0.5rem 0;">No forwarding numbers saved yet. Add one to make whitelist setup faster.</div>`;
      } else {
        container.innerHTML = `
          <div style="display: flex; flex-direction: column; gap: 0.5rem; margin-top: 0.5rem;">
            ${numbers.map(n => `
              <div data-id="${n.id}" style="display: flex; align-items: center; gap: 0.75rem; padding: 0.6rem 0.75rem; background: var(--bg-secondary, #f9fafb); border-radius: 8px; font-size: 0.875rem;">
                <span style="font-family: monospace; font-weight: 500;">${escapeHtml(formatPhoneNumber(n.number) || n.number)}</span>
                ${n.label ? `<span style="color: var(--text-secondary); font-size: 0.8rem;">${escapeHtml(n.label)}</span>` : ''}
                ${n.is_default ? `<span style="font-size: 0.7rem; font-weight: 700; background: #dcfce7; color: #16a34a; padding: 0.15rem 0.5rem; border-radius: 4px;">DEFAULT</span>` : ''}
                <div style="margin-left: auto; display: flex; gap: 0.25rem;">
                  ${!n.is_default ? `<button class="btn btn-sm fn-set-default-btn" data-id="${n.id}" title="Make this my default">Set default</button>` : ''}
                  <button class="btn btn-sm fn-edit-btn" data-id="${n.id}">Edit</button>
                  <button class="btn btn-sm fn-delete-btn" data-id="${n.id}" data-number="${escapeHtml(n.number)}" onmouseenter="this.style.background='#fee2e2';this.style.color='#dc2626';this.style.borderColor='#fca5a5'" onmouseleave="this.style.background='';this.style.color='';this.style.borderColor=''">Remove</button>
                </div>
              </div>
            `).join('')}
          </div>`;

        container.querySelectorAll('.fn-set-default-btn').forEach(btn => {
          btn.addEventListener('click', () => this.setDefaultForwardingNumber(btn.dataset.id));
        });
        container.querySelectorAll('.fn-edit-btn').forEach(btn => {
          btn.addEventListener('click', () => this.openForwardingNumberModal(btn.dataset.id));
        });
        container.querySelectorAll('.fn-delete-btn').forEach(btn => {
          btn.addEventListener('click', () => {
            this.showConfirmModal({
              title: 'Remove forwarding number',
              message: `Remove ${formatPhoneNumber(btn.dataset.number) || btn.dataset.number} from your saved list? Whitelist entries using it stay put — this only removes it from the pick list.`,
              confirmText: 'Remove',
              onConfirm: () => this.deleteForwardingNumber(btn.dataset.id),
            });
          });
        });
      }

      // Wire the "+ Add" button (bind once per render of the tab).
      const addBtn = document.getElementById('fn-add-btn');
      if (addBtn && !addBtn._fnBound) {
        addBtn._fnBound = true;
        addBtn.addEventListener('click', () => this.openForwardingNumberModal(null));
      }

      // Wire the FN modal form + dismiss handlers (bind once). All
      // dismiss paths (backdrop, ×, Cancel, stopPropagation on inner)
      // gate on this._fnIsSaving so the modal can't be closed
      // mid-POST. Inner modal's click swallow is preserved so clicks
      // inside the form don't bubble to the overlay handler.
      const form = document.getElementById('fn-form');
      const overlay = document.getElementById('fn-modal-overlay');
      const inner = document.getElementById('fn-modal-inner');
      const closeBtn = document.getElementById('fn-modal-close');
      const cancelBtn = document.getElementById('fn-modal-cancel');
      const dismiss = () => { if (!this._fnIsSaving) overlay.style.display = 'none'; };
      if (form && !form._fnBound) {
        form._fnBound = true;
        form.addEventListener('submit', (e) => {
          e.preventDefault();
          this.submitForwardingNumberForm();
        });
      ...
      // Now that forwarding numbers are loaded, refresh the whitelist
      // add modal's forward-to combobox so it reflects the latest list
      // + default.
      this._refreshWhitelistForwardSelect();
    } catch (err) {
      console.error('loadForwardingNumbers error:', err);
      container.innerHTML = `<div style="color: var(--danger); font-size: 0.875rem;">Failed to load saved forwarding numbers.</div>`;
    }
  },

  openForwardingNumberModal(id) {
    const modal = document.getElementById('fn-modal-overlay');
    const numberInput = document.getElementById('fn-number');
    const labelInput = document.getElementById('fn-label');
    const defaultInput = document.getElementById('fn-is-default');
    const title = document.getElementById('fn-modal-title');
    const saveBtn = document.getElementById('fn-save-btn');
    if (!modal) return;

    if (id) {
      const fn = (this._forwardingNumbers || []).find(n => n.id === id);
      if (!fn) return;
      title.textContent = 'Edit forwarding number';
      numberInput.value = fn.number;
      numberInput.disabled = true; // can't change the number in-place; remove + re-add instead
      labelInput.value = fn.label || '';
      defaultInput.checked = !!fn.is_default;
      saveBtn.dataset.id = id;
    } else {
      title.textContent = 'Add forwarding number';
      numberInput.value = '';
      numberInput.disabled = false;
      labelInput.value = '';
      defaultInput.checked = (this._forwardingNumbers || []).length === 0; // first one auto-default
  ...
  async submitForwardingNumberForm() {
    const numberInput = document.getElementById('fn-number');
    const labelInput = document.getElementById('fn-label');
    const defaultInput = document.getElementById('fn-is-default');
    const saveBtn = document.getElementById('fn-save-btn');
    ...
    try {
      ...
      let res, json;
      if (id) {
        // PATCH — only label + is_default can change
        res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-forwarding-numbers`, {
          method: 'PATCH',
          headers: { 'Authorization': `Bearer ${session.access_token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({ id, label: labelInput.value.trim() || null, is_default: defaultInput.checked }),
        });
      } else {
        // POST — number gets normalized server-side; client-side check
        // for UX so we fail fast.
        ...
        res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-forwarding-numbers`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${session.access_token}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({ number: normalized, label: labelInput.value.trim() || null, is_default: defaultInput.checked }),
        });
      }
      json = await res.json();
      if (!res.ok) {
        ...
        document.getElementById('fn-modal-overlay').style.display = 'none';
        showToast(id ? 'Forwarding number updated' : 'Forwarding number added', 'success');
        await this.loadForwardingNumbers();
      }
    } catch (err) {
      console.error('submitForwardingNumberForm error:', err);
      showToast('Failed to save', 'error');
    } finally {
      saveBtn.disabled = false;
      saveBtn.textContent = 'Save';
      this._fnIsSaving = false;
    }
  },

  async setDefaultForwardingNumber(id) {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) { showToast('Session expired — please refresh', 'error'); return; }
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-forwarding-numbers`, {
      ...
      if (!res.ok) {
        ...
        showToast(j.error?.message || 'Failed to set default', 'error');
        return;
      }
  ...
  async deleteForwardingNumber(id) {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) { showToast('Session expired — please refresh', 'error'); return; }
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-forwarding-numbers?id=${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${session.access_token}` },
      });
      if (!res.ok) {
      ...
      showToast('Forwarding number removed', 'success');
      await this.loadForwardingNumbers();
    } catch (err) {
      showToast('Failed to remove number', 'error');
    }
  ...
  // Build the whitelist add-modal's Forward-To <select> from the
  // current saved list. Called whenever the list changes so the modal
  // always reflects reality next time it opens.
  _refreshWhitelistForwardSelect() {
    const select = document.getElementById('whitelist-forward-select');
    if (!select) return;
    const numbers = this._forwardingNumbers || [];
    const defaultId = numbers.find(n => n.is_default)?.id;
    select.innerHTML = numbers.length
      ? numbers.map(n => `
          <option value="${n.id}" ${n.id === defaultId ? 'selected' : ''}>
  ...
  async loadWhitelist() {
    ...
    try {
      ...
      if (entries.length === 0) {
        ...
        const inputStyle = `border: none; background: transparent; font-size: 0.875rem; width: 100%; outline: none; padding: 0; border-radius: 4px;`;
        const inputFocus = `this.style.background='var(--bg-primary, #fff)'; this.style.outline='2px solid var(--primary, #6366f1)'; this.style.padding='0 4px';`;
        const inputBlur = `this.style.background='transparent'; this.style.outline='none'; this.style.padding='0';`;
        ...
        container.innerHTML = `
          <div style="margin-top: 0.75rem; display: flex; flex-direction: column; gap: 0.5rem;">
            ${entries.map(e => {
              const cooldownActive = e.cooldown_until && new Date(e.cooldown_until).getTime() > nowMs;
              const rowBg = cooldownActive ? '#fef3c7' : 'var(--bg-secondary, #f9fafb)';
              const cooldownBanner = cooldownActive ? `
                <div style="display: flex; align-items: center; justify-content: space-between; gap: 0.75rem; padding: 0.4rem 0.6rem; margin-top: 0.4rem; background: #fde68a; border-radius: 6px; font-size: 0.8125rem; color: #78350f;">
                  <span><strong>In cooldown</strong> — auto-clears at ${escapeHtml(fmtClock(e.cooldown_until))}. Forwarding is paused for this caller after a suspected spoofing burst. Calls fall through to the AI agent until then.</span>
                  <button class="btn btn-sm whitelist-clear-cooldown-btn" data-id="${e.id}" style="background:#fff; border:1px solid #d97706; color:#92400e; padding:0.25rem 0.6rem; white-space: nowrap;">Clear now</button>
                </div>
              ` : '';
              return `
              <div data-id="${e.id}" style="display: flex; flex-direction: column; padding: 0.6rem 0.75rem; background: ${rowBg}; border-radius: 8px; font-size: 0.875rem;">
                <div style="display: grid; grid-template-columns: repeat(5, 1fr); align-items: center;">
                  <input class="wl-label" data-id="${e.id}" data-field="label" value="${escapeHtml(e.label || '')}" placeholder="—" maxlength="100" style="${inputStyle} color: var(--text-secondary);" onfocus="${inputFocus}" onblur="${inputBlur}">
                  <input class="wl-caller" data-id="${e.id}" data-field="caller_number" value="${escapeHtml(e.caller_number)}" style="${inputStyle} text-align: center; font-family: monospace;" onfocus="${inputFocus}" onblur="${inputBlur}">
                  ...
                  <input class="wl-forward" data-id="${e.id}" data-field="forward_to" value="${escapeHtml(e.forward_to)}" style="${inputStyle} text-align: center; font-family: monospace;" onfocus="${inputFocus}" onblur="${inputBlur}">
        ...
        container.querySelectorAll('.whitelist-clear-cooldown-btn').forEach(btn => {
          btn.addEventListener('click', () => this.clearWhitelistCooldown(btn.dataset.id));
        });

        container.querySelectorAll('.wl-label, .wl-caller, .wl-forward').forEach(input => {
          const original = input.value;
          input._original = original;
          input.addEventListener('blur', () => this.saveWhitelistField(input));
          input.addEventListener('keydown', (e) => { if (e.key === 'Enter') input.blur(); if (e.key === 'Escape') { input.value = input._original; input.blur(); } });
        });
      }
      ...
      if (addBtn && !addBtn._whitelistBound) {
        ...
        addBtn.addEventListener('click', async () => {
          // Re-fetch saved forwarding numbers so the combobox reflects
          // anything added in another tab / device since this tab
          // loaded the agent. Without this the picker can be stale.
          await this.loadForwardingNumbers();
          // Reset form state every time we open the modal.
          document.getElementById('whitelist-add-form').reset();
          this._refreshWhitelistForwardSelect();
          const newInput = document.getElementById('whitelist-forward-new');
          if (newInput) { newInput.style.display = 'none'; newInput.value = ''; }
          document.getElementById('whitelist-modal-overlay').style.display = 'flex';
          setTimeout(() => document.getElementById('whitelist-caller')?.focus(), 50);
        });
      }

      // Forward-to select: when user picks "Enter a different number",
      // reveal the inline phone input. Hide it otherwise.
      const forwardSelect = document.getElementById('whitelist-forward-select');
      if (forwardSelect && !forwardSelect._whitelistBound) {
        forwardSelect._whitelistBound = true;
        forwardSelect.addEventListener('change', () => {
          const newInput = document.getElementById('whitelist-forward-new');
          const isNew = forwardSelect.value === '__new__';
      ...
      // Normalize caller_number on blur for instant feedback.
      const callerInput = document.getElementById('whitelist-caller');
      if (callerInput && !callerInput._whitelistBound) {
        callerInput._whitelistBound = true;
        callerInput.addEventListener('blur', () => {
          if (!callerInput.value.trim()) return;
          const normalized = normalizeE164(callerInput.value);
          const hint = document.getElementById('whitelist-caller-hint');
          if (normalized) {
            callerInput.value = formatPhoneNumber(normalized) || normalized;
            callerInput._normalized = normalized;
            if (hint) { hint.textContent = `Saved as ${normalized}`; hint.style.color = 'var(--text-secondary)'; }
          } else {
            callerInput._normalized = null;
            if (hint) { hint.textContent = `Doesn't look like a valid phone number — try (604) 555-1234.`; hint.style.color = 'var(--danger)'; }
          }
        });
      }

      // Same for the "Enter different number" inline forward input.
      const newForward = document.getElementById('whitelist-forward-new');
      if (newForward && !newForward._whitelistBound) {
        newForward._whitelistBound = true;
        newForward.addEventListener('blur', () => {
          if (!newForward.value.trim()) return;
          const normalized = normalizeE164(newForward.value);
          const hint = document.getElementById('whitelist-forward-hint');
          if (normalized) {
            newForward.value = formatPhoneNumber(normalized) || normalized;
            newForward._normalized = normalized;
            if (hint) { hint.textContent = `Saved as ${normalized} — will be added to your pick list.`; hint.style.color = 'var(--text-secondary)'; }
          } else {
            newForward._normalized = null;
            if (hint) { hint.textContent = `Doesn't look like a valid phone number — try (604) 555-1234.`; hint.style.color = 'var(--danger)'; }
          }
        });
      }
  ...
  async addWhitelistEntry() {
    const callerInput = document.getElementById('whitelist-caller');
    const forwardSelect = document.getElementById('whitelist-forward-select');
    const forwardNewInput = document.getElementById('whitelist-forward-new');
    const label = document.getElementById('whitelist-label').value.trim();
    const saveBtn = document.getElementById('whitelist-save-btn');

    // Normalize caller — use cached normalized value from blur if
    // present, otherwise normalize fresh.
    const callerNormalized = callerInput._normalized || normalizeE164(callerInput.value);
    if (!callerNormalized) {
      showToast('Enter a valid caller phone number, e.g. (604) 555-1234', 'error');
      callerInput.focus();
      return;
    }

    // Resolve forward target: either a saved-list pick, or the new
    // inline input which we'll normalize + let the backend upsert into
    // the pick list automatically.
    let forwardNormalized;
    if (forwardSelect.value === '__new__') {
      forwardNormalized = forwardNewInput._normalized || normalizeE164(forwardNewInput.value);
      if (!forwardNormalized) {
        showToast('Enter a valid forward-to phone number, e.g. (604) 555-1234', 'error');
        forwardNewInput.focus();
        return;
      }
    } else if (forwardSelect.value) {
      const picked = (this._forwardingNumbers || []).find(n => n.id === forwardSelect.value);
      if (!picked) {
        showToast('Pick a forward-to number, or choose "Enter a different number"', 'error');
        return;
      }
      forwardNormalized = picked.number;
    } else {
      showToast('Pick a forward-to number, or choose "Enter a different number"', 'error');
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-call-whitelist`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${session.access_token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ agent_id: this.agent.id, caller_number: callerNormalized, forward_to: forwardNormalized, label: label || null }),
      });
      const json = await res.json();
      ...
      if (!res.ok) {
        ...
        document.getElementById('whitelist-add-form').reset();
        callerInput._normalized = null;
        if (forwardNewInput) forwardNewInput._normalized = null;
        showToast('Whitelist entry added', 'success');
        // Refresh both — the backend auto-upserts a new forward_to into
        // the pick list, so the saved-numbers section should show it.
        await this.loadForwardingNumbers();
        await this.loadWhitelist();
      }
    } catch (err) {
  ...
  async saveWhitelistField(input) {
    if (input._saving) return;
    const newVal = input.value.trim();
    if (newVal === input._original) return;

    // For phone fields, normalize before sending. Reflect the
    // normalized version back into the input so the user sees the
    // canonical form. Reject only when normalization fails outright.
    let payloadValue = newVal;
    if (input.dataset.field === 'caller_number' || input.dataset.field === 'forward_to') {
      const normalized = normalizeE164(newVal);
      if (!normalized) {
        showToast('Doesn\'t look like a valid phone number — try (604) 555-1234', 'error');
        input.value = input._original;
        return;
      }
      // If the user retyped the same number in a different format
      // (e.g. paste of "604-562-8647" over an existing "+16045628647"),
      // normalization will produce the existing canonical value.
      // Skip the round-trip PATCH and just snap the display back.
      if (normalized === input._original) {
        input.value = input._original;
        return;
      }
    ...
    if (input.dataset.field === 'label' && newVal.length > 100) {
      showToast('Label must be 100 characters or fewer', 'error');
      input.value = input._original;
      return;
    }
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-call-whitelist`, {
        method: 'PATCH',
        headers: { 'Authorization': `Bearer ${session.access_token}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ id: input.dataset.id, [input.dataset.field]: payloadValue }),
      });
      const json = await res.json();
      if (!res.ok) {
        showToast(json.error?.message || 'Failed to save', 'error');
        ...
        // If user edited forward_to inline, the backend may have just
        // upserted a new pick-list entry. Refresh.
        if (input.dataset.field === 'forward_to') {
  ...
  async clearWhitelistCooldown(id) {
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) { showToast('Session expired — please refresh', 'error'); return; }
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/manage-call-whitelist`, {
      ...
      showToast('Cooldown cleared — forwarding re-enabled', 'success');
      await this.loadWhitelist();
    } catch (err) {
      showToast('Failed to clear cooldown', 'error');
    }
  ...
  // _esc removed — callers now use the shared escapeHtml from
  // lib/formatters.js, which covers &, <, >, ", and ' uniformly.
