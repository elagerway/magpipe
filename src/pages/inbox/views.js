...
export const viewsMethods = {
  ...
  renderSmsMessage(msg) {
    ...
    // Inbound photos (WhatsApp/MMS) re-hosted to the private media bucket and
    // recorded on the message as metadata.media with a long-lived signed URL.
    const mediaItems = Array.isArray(msg.metadata?.media) ? msg.metadata.media : [];
    ...
    const contentText = (mediaItems.length && (msg.content === '[image]' || !msg.content)) ? '' : msg.content;

    return `
      <div class="message-bubble ${isInbound ? 'inbound' : 'outbound'} ${isAI ? 'ai-message' : ''} ${isHuman ? 'human-message' : ''}" data-message-id="${msg.id}">
        ${contentText ? `<div class="message-content">${this.linkifyPhoneNumbers(contentText)}</div>` : ''}
        ${mediaHtml}
        ${translationHtml}
