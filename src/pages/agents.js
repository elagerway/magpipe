...
export default class AgentsPage {
  ...
  async render() {
    ...
    // Auto-fetch missing avatars in the background
  ...
  async fetchMissingAvatars() {
    const agentsMissingAvatars = this.agents.filter(a => a.voice_id && !a.avatar_url);
    if (agentsMissingAvatars.length === 0) return;
    ...
    try {
      ...
      // Fetch avatars for each missing agent in parallel
      const responses = await Promise.allSettled(
        agentsMissingAvatars.map(a =>
          fetch(`${supabaseUrl}/functions/v1/fetch-agent-avatar`, {
            method: 'POST',
            headers,
      ...
      // Page was navigated away — bail out
      if (!this.userId) return;

      let updated = false;
      for (const res of responses) {
        if (res.status !== 'fulfilled' || !res.value?.results) continue;
        for (const result of res.value.results) {
          if (result.avatar_url) {
            const agent = this.agents.find(a => a.id === result.agent_id);
      ...
      // Silent fail — avatars are cosmetic
      console.warn('Failed to auto-fetch avatars:', err);
    }
  }
