...
export const supportTabMethods = {
  ...
  async renderSupportContent() {
    ...
    container.innerHTML = `
      ...
      <div class="support-subtabs">
        ...
        <button class="support-subtab ${this.supportSubTab === 'latency' ? 'active' : ''}" data-support-subtab="latency">Latency</button>
      ...
      <!-- Latency sub-tab -->
      <div id="support-subtab-latency" class="support-subtab-content" style="display: ${this.supportSubTab === 'latency' ? 'flex' : 'none'};">
        <div class="loading-spinner">Loading latency…</div>
      </div>
    ...
    const allSupportPanes = ['tickets', 'users', 'global-agent', 'chat', 'settings', 'errors', 'latency', 'numbers'];
  ...
  async _loadSupportSubtab(subtab) {
    ...
    try {
      ...
      } else if (subtab === 'latency') {
        await this.renderLatencyTab();
  ...
  async openSupportThread(threadId, currentStatus) {
    ...
    try {
      ...
      // Reply-all: figure out who else was on the inbound message (To + Cc),
      // minus our own support mailbox addresses and the primary recipient.
      // ownAddresses comes from the support email config (server-authoritative),
      // so it tracks the connected mailbox/agent automatically.
      const ownAddresses = (data.ownAddresses || []).map(a => (a || '').toLowerCase());
      const extractEmails = (h) => h ? (h.match(/[^\s<>,;"]+@[^\s<>,;"]+/g) || []).map(e => e.trim().toLowerCase()) : [];
      const inboundForCc = messages.find(m => m.direction === 'inbound') || firstMsg;
      const primaryTo = (inboundForCc.from_email || '').toLowerCase();
      const ccCandidates = [...extractEmails(inboundForCc.to_email), ...extractEmails(inboundForCc.cc_email)]
        .filter((e, i, arr) => !ownAddresses.includes(e) && e !== primaryTo && arr.indexOf(e) === i);
      const hasReplyAll = ccCandidates.length > 0;
      const replyAllLabel = hasReplyAll ? `Reply all (+${ccCandidates.length} — ${ccCandidates.join(', ')})` : '';
      ...
      threadView.innerHTML = `
        ...
        ${pendingDraft ? `
          <div class="tv-draft">
            ...
            <div class="tv-draft-actions" style="display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;">
              ...
              ${hasReplyAll ? `<label title="${escapeHtml(ccCandidates.join(', '))}" style="display:flex;align-items:center;gap:0.4rem;font-size:0.8rem;color:var(--text-secondary);cursor:pointer;">
                <input type="checkbox" id="approve-reply-all-checkbox" /> ${escapeHtml(replyAllLabel)}
              </label>` : ''}
            </div>
          </div>
        ` : ''}
        ...
        <div class="tv-reply">
          ...
          <div class="tv-reply-footer" style="display:flex;align-items:center;gap:0.75rem;">
            ${hasReplyAll ? `<label title="${escapeHtml(ccCandidates.join(', '))}" style="display:flex;align-items:center;gap:0.4rem;font-size:0.8rem;color:var(--text-secondary);cursor:pointer;margin-right:auto;">
              <input type="checkbox" id="reply-all-checkbox" /> ${escapeHtml(replyAllLabel)}
            </label>` : ''}
          ...
          const res = await fetch(
            `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/support-tickets-api`,
            {
              ...
              body: JSON.stringify({
                ...
                replyAll: document.getElementById('reply-all-checkbox')?.checked || false,
              }),
            }
          );
        ...
        try {
          const res = await fetch(
            `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/support-tickets-api`,
            {
              ...
              body: JSON.stringify({ action: 'approve_draft', ticketId, replyAll: document.getElementById('approve-reply-all-checkbox')?.checked || false }),
            }
          );
        ...
        <div class="tl-toolbar" style="margin-bottom: 1rem; flex-wrap: wrap; gap: 0.5rem;">
          ...
          <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; align-items: center; margin-left: auto;">
            ...
            <select id="errors-type-filter" class="form-input form-select" style="max-width: 200px; font-size: 0.85rem;">
              ...
              <option value="meta_whatsapp_delivery_failure">WA Delivery Failed</option>
              <option value="voice_forward_failure">Voice Forward Failed</option>
              <option value="voice_call_failure">Voice Call Failed</option>
            </select>
  ...
  async _loadErrorLogs() {
    ...
    try {
          ...
          <table class="data-table" style="width: 100%; font-size: 0.85rem;">
            ...
            <tbody>
              ${errors.map((err, idx) => {
                ...
                const userPhone = err.users?.phone_number || '';
                ...
                const hasDetails = Object.keys(meta).length > 0 || err.error_code || err.user_id;
                return `
                  <tr class="error-row" data-idx="${idx}" style="cursor:pointer;" title="Click to expand">
                    <td style="white-space: nowrap; color: var(--text-muted);">${dateStr}<br><span style="font-size: 0.75rem;">${timeStr}</span></td>
                    <td>${severityBadge}</td>
                    <td>${sourceBadge}</td>
                    <td>${typeBadge}</td>
                    <td style="max-width: 320px;">
                      <div style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${escapeHtml(err.error_message)}</div>
                    </td>
                  ...
                  <tr class="error-detail-row" id="error-detail-${idx}" style="display:none;">
                    <td colspan="6" style="padding:0.75rem 1rem;background:var(--bg-secondary);border-top:none;">
                      <div style="font-size:0.8rem;display:grid;grid-template-columns:auto 1fr;gap:0.35rem 1rem;max-width:700px;">
                        <span style="font-weight:600;color:var(--text-muted);">Message</span>
                        <span style="word-break:break-word;">${escapeHtml(err.error_message)}</span>
                        ${err.error_code ? `
                          <span style="font-weight:600;color:var(--text-muted);">Code</span>
                          <span style="font-family:monospace;">${escapeHtml(err.error_code)}</span>
                        ` : ''}
                        <span style="font-weight:600;color:var(--text-muted);">Time</span>
                        <span>${date.toLocaleString()}</span>
                        <span style="font-weight:600;color:var(--text-muted);">Type</span>
                        <span>${escapeHtml(err.error_type)}</span>
                        <span style="font-weight:600;color:var(--text-muted);">Source</span>
                        <span>${escapeHtml(err.source || 'supabase')}</span>
                        ${err.user_id ? `
                          <span style="font-weight:600;color:var(--text-muted);">User</span>
                          <span>${escapeHtml(userEmail || err.user_id)}${userPhone ? ` (${escapeHtml(userPhone)})` : ''}</span>
                        ` : ''}
                        ${Object.entries(meta).map(([k, v]) => `
                          <span style="font-weight:600;color:var(--text-muted);">${escapeHtml(k)}</span>
                          <span style="word-break:break-all;font-family:${typeof v === 'string' && v.length > 50 ? 'monospace;font-size:0.75rem' : 'inherit'};">${escapeHtml(typeof v === 'object' ? JSON.stringify(v, null, 2) : String(v))}</span>
                        `).join('')}
                      </div>
                    </td>
                  </tr>
      ...
      container.querySelectorAll('.error-row').forEach(row => {
        row.addEventListener('click', (e) => {
          if (e.target.closest('.error-user-link')) return;
          const idx = row.dataset.idx;
          const detail = document.getElementById(`error-detail-${idx}`);
          if (!detail) return;
          const isOpen = detail.style.display !== 'none';
          // Close all others
          container.querySelectorAll('.error-detail-row').forEach(r => r.style.display = 'none');
          container.querySelectorAll('.error-row').forEach(r => r.style.background = '');
          if (!isOpen) {
  ...
  _errorTypeBadge(type) {
    const map = {
      ...
      meta_whatsapp_delivery_failure: { label: 'WA Delivery', bg: '#fee2e2', color: '#991b1b' },
      voice_forward_failure:   { label: 'Voice Forward', bg: '#fee2e2', color: '#991b1b' },
      voice_call_failure:      { label: 'Voice Call',  bg: '#fee2e2', color: '#991b1b' },
    };
    const b = map[type] || { label: type.replace(/_/g, ' '), bg: '#f3f4f6', color: '#374151' };
    return `<span style="display:inline-block;padding:0.15rem 0.5rem;border-radius:9999px;font-size:0.75rem;font-weight:500;background:${b.bg};color:${b.color};white-space:nowrap;">${b.label}</span>`;
  },

  // ── Latency sub-tab (voice PSTN→Agent response gap, per turn) ───────────
  async renderLatencyTab() {
    const content = document.getElementById('admin-tab-content');
    if (!content) return;

    this._latencyPage = 1;
    this._latencyPerPage = 25;
    this._latencyWindowHours = this._latencyWindowHours || 24;

    content.innerHTML = `
      <div class="support-section">
        <div class="tl-toolbar" style="margin-bottom: 1rem; flex-wrap: wrap; gap: 0.5rem;">
          <h3 style="margin: 0;">Voice Latency <span style="font-weight:400;color:var(--text-muted);font-size:0.8rem;">end of caller speech → agent responds</span></h3>
          <div style="display: flex; gap: 0.5rem; flex-wrap: wrap; align-items: center; margin-left: auto;">
            <select id="latency-window-filter" class="form-input form-select" style="max-width: 140px; font-size: 0.85rem;">
              <option value="24">Last 24h</option>
              <option value="168">Last 7d</option>
              <option value="720">Last 30d</option>
            </select>
            <button class="btn btn-secondary" id="latency-refresh-btn" style="font-size: 0.85rem; padding: 0.35rem 0.75rem; white-space: nowrap;">↻ Refresh</button>
          </div>
        </div>
        <div id="latency-summary-cards" style="display:flex;flex-wrap:wrap;gap:0.75rem;margin-bottom:1rem;"></div>
    ...
    const winSel = document.getElementById('latency-window-filter');
    if (winSel) winSel.value = String(this._latencyWindowHours);
    winSel?.addEventListener('change', () => {
      this._latencyWindowHours = parseInt(winSel.value, 10) || 24;
      this._latencyPage = 1;
      this._loadLatencyMetrics();
    });
    document.getElementById('latency-refresh-btn')?.addEventListener('click', () => this._loadLatencyMetrics());

    await this._loadLatencyMetrics();
  },

  _latencyColor(ms) {
    if (ms == null) return 'var(--text-muted)';
    if (ms < 800) return '#166534';   // green: snappy
    if (ms < 1500) return '#854d0e';  // amber: noticeable
    return '#991b1b';                 // red: bad QoS
  },
  ...
  _latencyStatCard(label, value, unit, color) {
    return `
      <div style="flex:1;min-width:150px;background:var(--bg-secondary);border-radius:8px;padding:0.75rem 1rem;">
        <div style="font-size:0.75rem;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.03em;">${label}</div>
        <div style="font-size:1.5rem;font-weight:600;color:${color || 'var(--text)'};">${value == null ? '—' : value}<span style="font-size:0.85rem;font-weight:400;color:var(--text-muted);"> ${value == null ? '' : unit}</span></div>
      </div>
    `;
  },

  async _loadLatencyMetrics() {
    const container = document.getElementById('latency-table-container');
    const cards = document.getElementById('latency-summary-cards');
    if (!container) return;
    container.innerHTML = '<div class="loading-spinner">Loading latency…</div>';

    try {
      const sinceIso = new Date(Date.now() - this._latencyWindowHours * 3600 * 1000).toISOString();

      // Pull recent turns within the window (cap 1000) — aggregates computed
      // client-side; table paginated client-side from the same set. count:exact
      // returns the true window total (ignoring the limit) so we can tell the
      // operator when the cards reflect only the most recent sample.
      ...
      const all = rows || [];
      const windowTotal = count == null ? all.length : count;
      const capped = windowTotal > all.length;

      if (cards) {
        const e2e = all.map(r => r.e2e_latency_ms).filter(v => typeof v === 'number').sort((a, b) => a - b);
        const avg = arr => arr.length ? Math.round(arr.reduce((s, v) => s + v, 0) / arr.length) : null;
        const avgOf = key => avg(all.map(r => r[key]).filter(v => typeof v === 'number'));
        const e2eAvg = avg(e2e);
        const e2eP95 = this._pct(e2e, 95);
        const cardsHtml =
          this._latencyStatCard('Turns', windowTotal, '', null) +
          this._latencyStatCard('e2e avg', e2eAvg, 'ms', this._latencyColor(e2eAvg)) +
          this._latencyStatCard('e2e p95', e2eP95, 'ms', this._latencyColor(e2eP95)) +
          this._latencyStatCard('turn detect avg', avgOf('end_of_turn_delay_ms'), 'ms', null) +
          this._latencyStatCard('LLM ttft avg', avgOf('llm_ttft_ms'), 'ms', null) +
          this._latencyStatCard('TTS ttfb avg', avgOf('tts_ttfb_ms'), 'ms', null);
        // Be explicit when avg/p95 are over the most-recent sample, not the
        // full window — otherwise a high-volume window silently understates.
        const note = capped
          ? `<div style="flex-basis:100%;font-size:0.75rem;color:var(--text-muted);">averages &amp; p95 over the most recent ${all.length.toLocaleString()} of ${windowTotal.toLocaleString()} turns in this window</div>`
          : '';
        cards.innerHTML = cardsHtml + note;
      }

      if (all.length === 0) {
        container.innerHTML = `
          <div class="detail-placeholder" style="padding: 2rem; text-align: center; color: var(--text-muted);">
            No latency data in this window yet. New calls populate it as turns complete.
          </div>
        `;
        return;
      }

      const totalPages = Math.ceil(all.length / this._latencyPerPage);
      const from = (this._latencyPage - 1) * this._latencyPerPage;
      const pageRows = all.slice(from, from + this._latencyPerPage);
      const cell = (ms) => `<td style="text-align:right;font-variant-numeric:tabular-nums;color:${this._latencyColor(ms)};">${ms == null ? '—' : ms}</td>`;
      ...
      container.innerHTML = `
          ...
          <table class="data-table" style="width: 100%; font-size: 0.85rem;">
            <thead>
              <tr>
                ...
                <th style="text-align:right;" title="Turn detection / endpointing">turn</th>
                <th style="text-align:right;" title="LLM time-to-first-token">ttft</th>
                <th style="text-align:right;" title="TTS time-to-first-byte">ttfb</th>
                <th>Models</th>
              </tr>
            </thead>
            <tbody>
              ${pageRows.map(r => {
                ...
                return `
                  <tr>
                    <td style="white-space: nowrap; color: var(--text-muted);">${dateStr} <span style="font-size:0.75rem;">${timeStr}</span></td>
                    <td style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${escapeHtml(email || '—')}</td>
                    <td>${escapeHtml(r.direction || '—')}</td>
                    ${cell(r.e2e_latency_ms)}
                    ${cell(r.end_of_turn_delay_ms)}
                    ${cell(r.llm_ttft_ms)}
                    ${cell(r.tts_ttfb_ms)}
                    <td style="font-size:0.75rem;color:var(--text-muted);white-space:nowrap;">${escapeHtml(r.llm_model || '?')} / ${escapeHtml(r.tts_model || '?')}</td>
                  </tr>
                `;
              }).join('')}
            </tbody>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 1rem; padding: 0.5rem 0;">
          <span style="font-size: 0.8rem; color: var(--text-muted);">${all.length} turns${all.length >= 1000 ? '+ (capped)' : ''}${totalPages > 1 ? ` — page ${this._latencyPage} of ${totalPages}` : ''}</span>
          ${totalPages > 1 ? `
            <div style="display: flex; gap: 0.5rem;">
              <button class="btn btn-secondary" id="latency-prev-btn" style="font-size: 0.8rem; padding: 0.3rem 0.75rem;" ${this._latencyPage <= 1 ? 'disabled' : ''}>Prev</button>
              <button class="btn btn-secondary" id="latency-next-btn" style="font-size: 0.8rem; padding: 0.3rem 0.75rem;" ${this._latencyPage >= totalPages ? 'disabled' : ''}>Next</button>
            </div>
          ` : ''}
        </div>
      `;

      document.getElementById('latency-prev-btn')?.addEventListener('click', () => {
        if (this._latencyPage > 1) { this._latencyPage--; this._loadLatencyMetrics(); }
      });
      document.getElementById('latency-next-btn')?.addEventListener('click', () => {
        if (this._latencyPage < totalPages) { this._latencyPage++; this._loadLatencyMetrics(); }
      });
    } catch (err) {
      console.error('Error loading latency metrics:', err);
      container.innerHTML = `
        <div class="detail-placeholder" style="padding: 2rem; text-align: center; color: var(--text-muted);">
          Failed to load latency metrics: ${escapeHtml(err.message || String(err))}
        </div>
      `;
    }
  },
