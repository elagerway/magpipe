...
async function updateUnreadCount(userId) {
  ...
  try {
    ...
    // Get all inbound messages (same as inbox.js - no date filter)
    ...
    unreadCount = Object.values(unreadByConv).reduce((sum, count) => sum + count, 0);
  } catch (error) {
