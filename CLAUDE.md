...
## Database Management
- **Always handle database migrations and deployments**: When migrations are created or modified, automatically deploy them using `export SUPABASE_ACCESS_TOKEN=YOUR_SUPABASE_ACCESS_TOKEN && npx supabase db push` or `npx supabase db reset --linked` (with 'y' confirmation)
