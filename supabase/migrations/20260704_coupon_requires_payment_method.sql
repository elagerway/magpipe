-- Fraud guard: require a payment method on file before granting coupon credits.
-- Without this, a coupon like CONVEYR50 ($50) is a free-money faucet — create an
-- account, redeem, burn credits, repeat. users.has_payment_method is set by the
-- Stripe setup_intent.succeeded webhook. Enforced in the RPC (the atomic txn) so
-- the edge function, MCP, or any other caller can't bypass it.
...
BEGIN
  ...
  -- Fraud guard: card on file required before we hand out credits
