...
async function createSupportTicket(
  supabase: any,
  email: { from: string; fromName?: string; subject: string; textBody: string; htmlBody?: string; messageId?: string }
) {
  ...
  // Dedup: check if a ticket with this Postmark MessageID already exists
  if (email.messageId) {
    ...
    if (existing) {
      console.log(`Duplicate email ${email.messageId}, ticket ${existing.id} already exists`)
      return
    }
  }
  ...
  const threadId = `email-${crypto.randomUUID()}`
  ...
  const { data: ticket, error: ticketError } = await supabase
    ...
    .insert({
      ...
      gmail_message_id: email.messageId || null,
      from_email: email.from,
      from_name: email.fromName || null,
      to_email: 'help@magpipe.ai',
      subject: email.subject,
  ...
  console.log(`Support ticket created from inbound email: ${ticket.ticket_ref} from ${email.from}`)
}
