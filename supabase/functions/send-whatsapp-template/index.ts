...
Deno.serve(async (req) => {
  ...
  const { agent_id, to, template_name, language = 'en_US', components, metadata } = body
  ...
  if (!metaRes.ok) {
    const errStr = JSON.stringify(metaResult)
    console.error('Meta template send error:', errStr)
    await reportError(supabase, {
      error_type: 'meta_whatsapp_send_failure',
      error_message: errStr,
      error_code: String(metaRes.status),
      source: 'send-whatsapp-template',
      severity: 'error',
      metadata: { channel: 'whatsapp', phone_number_id: waAccount.phone_number_id, to, template_name, trigger: 'template' },
      user_id: user.id,
    }).catch(() => {})
    return new Response(JSON.stringify({
  ...
  const { error: insertError } = await supabase
    ...
    .insert({
      ...
      // Customer-supplied attribution metadata. The opener is a template, so
      // this is the row an in-thread reply points back to (Path B) — the reply
      // forward echoes this metadata for project attribution.
