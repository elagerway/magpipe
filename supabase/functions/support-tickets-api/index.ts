...
async function handleSendReply(supabase: any, body: any) {
  const { threadId, replyBody, replyBodyHtml, attachments } = body
  ...
  if (isGmailTicket) {
    ...
    // Build HTML content for sending (prefer rich HTML, fall back to plain text)
    const sendHtml = replyBodyHtml || replyBody
    ...
    await sendPostmarkReply(toEmail, replySubject, replyBody, replyBodyHtml)
...
async function sendPostmarkReply(toEmail: string, subject: string, body: string, htmlBody?: string) {
