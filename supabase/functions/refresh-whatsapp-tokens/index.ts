/**
 * DEPRECATED — DO NOT RE-SCHEDULE.
 *
 * This belonged to the old user-token model (tokens expired ~60 days, renewed
 * via fb_exchange_token). We now use a never-expiring System User token, so
 * there is nothing to refresh — and exchanging a system token here risks
 * downgrading it to an expiring one (the exact 2026-05-29 outage). The cron was
 * unscheduled in 20260529_unschedule_whatsapp_token_refresh.sql; token death is
 * now caught proactively by whatsapp-health-check. Kept only for history.
 *
