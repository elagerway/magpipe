...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (tool_name) {
      case 'list_contacts':
        response = await handleListContacts(supabase, user.id, args);
        break;

      case 'add_contact':
        response = await handleAddContact(supabase, user.id, args, mode);
        break;

      case 'call_contact':
        response = await handleCallContact(supabase, user.id, args, mode);
        break;

      case 'send_sms':
        response = await handleSendSms(supabase, user.id, args, mode);
        break;

      case 'schedule_sms':
        response = await handleScheduleSms(supabase, user.id, args, mode);
        break;
      ...
      case 'update_system_prompt':
        response = await handleUpdateSystemPrompt(supabase, user.id, args, mode);
        break;

      case 'add_knowledge_source':
        response = await handleAddKnowledgeSource(supabase, user.id, jwt, args, mode);
        break;

      case 'check_calendar_availability':
        response = await handleCheckCalendarAvailability(supabase, user.id, jwt, args);
        break;

      case 'book_calendar_appointment':
        response = await handleBookCalendarAppointment(supabase, user.id, jwt, args, mode);
        break;

      // Integration management tools
      case 'list_available_integrations':
        response = await handleListAvailableIntegrations(supabase, user.id);
        break;

      case 'start_integration_connection':
        response = await handleStartIntegrationConnection(supabase, user.id, jwt, args);
        break;

      case 'check_integration_status':
        response = await handleCheckIntegrationStatus(supabase, user.id, args);
        break;

      // Integration tools (future)
      case 'slack_send_message':
      case 'slack_list_channels':
        response = await handleIntegrationTool(supabase, user.id, tool_name, args);
        break;

      default:
        // Check if this is an MCP server tool (format: "server_slug:tool_name")
        if (tool_name.includes(':')) {
          response = await handleMcpServerTool(supabase, user.id, jwt, tool_name, args);
        } else {
