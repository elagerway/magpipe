...
Deno.serve(async (req) => {
  try {
    ...
    // Keep agent_email_configs history in sync (prevents reprocessing if re-enabled)
    await supabase
    ...
    for (const msg of newInboundMessages) {
      ...
      if (config.agent_mode === 'draft' || config.agent_mode === 'auto') {
        ...
        await generateAiDraft(supabase, accessToken, config, msg, integration.user_id)
      }
    }
...
async function generateAiDraft(supabase: any, accessToken: string, config: any, msg: any, userId?: string) {
