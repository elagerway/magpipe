  ...
  try {
    ...
    if (attachmentList.length > 0) {
      const items = attachmentList.map(a => {
        ...
        if (isVideo) {
          return `<div style="margin:0.5rem 0;"><a href="${a.url}" style="color:#6366f1;text-decoration:none;">&#9658; ${a.name || 'Video'}</a> <span style="color:#999;font-size:0.8rem;">(${(a.size / 1024 / 1024).toFixed(1)}MB)</span></div>`
        }
        return `<div style="margin:0.5rem 0;"><a href="${a.url}" target="_blank"><img src="${a.url}" alt="${a.name || 'Image'}" style="max-width:300px;max-height:200px;border-radius:6px;border:1px solid #ddd;"></a></div>`
      }).join('')
