  ...
  try {
    ...
    const { data: callRecord, error: insertError } = await serviceRoleClient
      ...
      .insert({
        ...
        // Store clean E.164 — call.completed webhook scoping matches this
        // against api_key_numbers (always clean E.164); an unnormalized
        // caller_id here would silently drop the customer's webhook.
    ...
    try {
      ...
      if (livekitUrl && livekitApiKey && livekitApiSecret) {
        roomClient = new RoomServiceClient(livekitUrl, livekitApiKey, livekitApiSecret);
    ...
    // PREWARM: hold the PSTN dial until the agent is fully warm so the callee gets an
    // INSTANT greeting instead of waiting out the ~6s session warmup. The agent stamps
    // call_records.agent_warmed_at once STT/LLM/TTS are connected (keyed on call_record_id,
    // so it works regardless of room/dispatch). Opt-in via body.prewarm — use it on
    // scheduled/automated paths that have lead time; omit on interactive UI calls where a
    // human is waiting. Falls through to an immediate ("cold") dial after a hard timeout so
    // a slow/missing agent never strands the call.
    if (prewarm) {
      const PREWARM_TIMEOUT_MS = 15000;
      const POLL_INTERVAL_MS = 300;
      const deadline = Date.now() + PREWARM_TIMEOUT_MS;
      let warmed = false;
      console.log(`🔥 Prewarm: waiting up to ${PREWARM_TIMEOUT_MS}ms for agent to warm (call_record ${callRecord.id})`);
      while (Date.now() < deadline) {
        const { data: rec } = await serviceRoleClient
          .from("call_records")
          .select("agent_warmed_at")
          .eq("id", callRecord.id)
          .maybeSingle();
        if (rec?.agent_warmed_at) { warmed = true; break; }
        await new Promise((r) => setTimeout(r, POLL_INTERVAL_MS));
      }
      console.log(warmed
        ? "🔥 Prewarm: agent warm — dialing PSTN now (instant greeting)"
        : `🔥 Prewarm: agent not warm within ${PREWARM_TIMEOUT_MS}ms — dialing PSTN anyway (cold fallback)`);
    }
