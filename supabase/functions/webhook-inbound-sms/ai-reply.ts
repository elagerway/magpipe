...
` : ''
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (!openaiResponse.ok) {
        ...
        await reportError(supabase, {
          ...
          source: 'webhook-inbound-sms/ai-reply',
          severity: 'error',
          metadata: { channel: 'sms', from, to, agent_id: agentConfig?.id, model: 'gpt-4o-mini' },
          user_id: userId,
        }).catch(() => {})
    ...
    reportError(supabase, {
      error_type: 'sms_ai_reply_failure',
      error_message: String((error as Error)?.message || error),
      error_code: 'processAndReplySMS',
      source: 'supabase',
      severity: 'error',
      metadata: { from, to, agent_id: agentConfig?.id },
      user_id: userId,
    }).catch(() => {})
  }
}
