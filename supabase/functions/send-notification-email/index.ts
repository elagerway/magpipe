...
import { buildEmailBody, callSummaryLine } from '../_shared/build-notification-body.ts'
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Which of the user's numbers rang and which agent answered — shown in the
    // default templates too, so a user who never opened "Customize what's
    // included" isn't left with an alert that can't be acted on.
    const dialledLabel = data.direction === 'outbound' ? 'From' : 'To'
    const callContextHtml =
      (data.serviceNumber ? `<p><strong>${dialledLabel}:</strong> ${data.serviceNumber}</p>` : '') +
      (data.agentName ? `<p><strong>Agent:</strong> ${data.agentName}</p>` : '')
    const callContextText =
      (data.serviceNumber ? `\n${dialledLabel}: ${data.serviceNumber}` : '') +
      (data.agentName ? `\nAgent: ${data.agentName}` : '')
    // Unknown-caller lookup is on by default for email (off for Slack).
    const escHtml = (t: string) => String(t).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')
    const lookupHtml = data.callerLookup
      ? `<p style="white-space: pre-line;">${escHtml(data.callerLookup)}</p>`
      : ''
    const lookupText = data.callerLookup ? `\n\n${data.callerLookup}` : ''
    ...
    if (!subject) switch (type) {
      case 'missed_call':
        ...
        `
        textBody = `You have a missed call from ${data.callerNumber || 'Unknown'} at ${new Date(data.timestamp).toLocaleString('en-US', { timeZone: userTimezone })}${callContextText}${lookupText}\n\n---\nNotification ID: ${notificationId}`
        break
      ...
      case 'completed_call':
        ...
        htmlBody = `
          ...
          <p><strong>Time:</strong> ${new Date(data.timestamp).toLocaleString('en-US', { timeZone: userTimezone })}</p>
          ${data.duration ? `<p><strong>Duration:</strong> ${data.duration} seconds</p>` : ''}
          <p><strong>Summary:</strong><br>${callSummaryLine(data).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</p>
          ...
          <p style="margin-top: 2rem; padding-top: 1rem; border-top: 1px solid #ccc; color: #666; font-size: 0.75rem;">Notification ID: ${notificationId}</p>
        `
        textBody = `Call ${data.successful ? 'completed' : 'ended'} with ${data.callerNumber || 'Unknown'} at ${new Date(data.timestamp).toLocaleString('en-US', { timeZone: userTimezone })}${callContextText}${lookupText}\n\nSummary:\n${callSummaryLine(data)}${data.recordingUrl && String(data.recordingUrl).startsWith('https://') ? `\nRecording: ${data.recordingUrl}` : ''}\n\n---\nNotification ID: ${notificationId}`
        break
